...
const (
	workerMachineConfigFileName = "99_openshift-installer-ignition_worker.yaml"
)
...
func (a *WorkerIgnitionCheck) Generate(dependencies asset.Parents) error {
	...
	if savedPointerIgnition != defaultPointerIgnition {
		...
		mc, err := generateMachineConfig(*savedPointerIgnition, "worker")
		if err != nil {
...
func (a *WorkerIgnitionCheck) Load(f asset.FileFetcher) (found bool, err error) {
	file, err := f.FetchByName(filepath.Join(directory, workerMachineConfigFileName))
	if err != nil {
