...
func TFVars(libvirtURI, apiVIP, imageCacheIP, bootstrapOSImage, externalBridge, externalMAC, provisioningBridge, provisioningMAC string, platformHosts []*baremetal.Host, image, ironicUsername, ironicPassword, ignition string) ([]byte, error) {
	...
	for _, host := range platformHosts {
		...
		driverInfo["deploy_kernel"] = fmt.Sprintf("http://%s/images/rhcos-latest-live.kernel", net.JoinHostPort(imageCacheIP, "80"))
		driverInfo["deploy_ramdisk"] = fmt.Sprintf("http://%s/images/rhcos-latest-live.initramfs", net.JoinHostPort(imageCacheIP, "80"))
		driverInfo["deploy_iso"] = fmt.Sprintf("http://%s/images/rhcos-latest-live.iso", net.JoinHostPort(imageCacheIP, "80"))
		...
		// in the future.
		var hostDeploySteps string
		if strings.Contains(accessDetails.BootInterface(), "redfish-virtual-media") {
